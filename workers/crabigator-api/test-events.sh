#!/bin/bash
# View SSE events from a crabigator session
# Usage: ./test-events.sh <session_id>
...
if [ -z "$SESSION_ID" ]; then
    echo "Usage: ./test-events.sh <session_id>"
    exit 1
fi
...
# Sign the request (GET for events)
method = "GET"
...
echo "Connecting to SSE stream for session: $SESSION_ID"
echo "Press Ctrl+C to stop"
echo "---"

# Stream SSE events
curl -N -s "https://drinkcrabigator.com${API_PATH}" \
