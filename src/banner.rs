...
use crate::terminal::escape::{
    BOLD, FG_CYAN, FG_GRAY, FG_GREEN, FG_ORANGE, FG_PURPLE, FG_RED, RESET,
};
...
pub fn print_session_banner(session_id: &str, platform: PlatformKind, cols: u16) {
    ...
    let date_str = format!(
        "{}, {} {}{}, {} {}",
        day, month, date_num, suffix, year, time
    );
    ...
    let right_side =
        format!("{FG_PURPLE}{platform_name}{RESET} {FG_GREEN}·{RESET} {FG_GREEN}{date_str}{RESET}");
...
pub fn print_session_end_line(platform: PlatformKind, cols: u16) {
    ...
    let platform_name = platform.display_name();
    let right_side =
        format!("{FG_PURPLE}{platform_name}{RESET} {FG_RED}·{RESET} {FG_RED}{date_str}{RESET}");
